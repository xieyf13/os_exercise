...
class fs:
    ...
    # DONE
        ...
        node = self.inodes[inum]
        if node.getRefCnt() == 1:
            self.dataFree(node.getAddr())
            self.inodeFree(inum)
        else:
            node.decRefCnt()
        pname = self.getParent(tfile)
        pnode = self.inodes[self.nameToInum[pname]]
        pnode.decRefCnt()
        self.data[pnode.getAddr()].delDirEntry(tfile)
    ...
    # DONE
        pnode = self.inodes[self.nameToInum[parent]]
        pblock = self.data[pnode.getAddr()]
        if pblock.getFreeEntries() <= 0 or pblock.dirEntryExists(newfile):
            return -1
        else:
            tinum = self.nameToInum[target]
            tnode = self.inodes[tinum]
            tnode.incRefCnt()
            pnode.incRefCnt()
    ...
    # DONE
        ...
        if pblock.getFreeEntries() <= 0 or pblock.dirEntryExists(newfile):
            ...
            pblock.addDirEntry(newfile, inum)
            pnode.incRefCnt()
            if ftype == 'd':
                ad = self.dataAlloc()
                ref = 2
                nblock = self.data[ad]
                nblock.setType(ftype)
                nblock.addDirEntry('.', inum)
                nblock.addDirEntry('..', self.nameToInum[parent])
            else:
                ad = -1
                ref = 1
            nnode = self.inodes[inum]
            nnode.setAll(ftype, ad, ref)
            return inum
    ...
    # DONE
        if curSize == 0: #empty
            data_addr = self.dataAlloc()
            self.inodes[inum].setAddr(data_addr)
            dblock = self.data[data_addr]
            dblock.setType('f')
            dblock.addData(data)
        else:
            return -1
    ...
    def createSoftLink(self, target, newfile, parent):
        ...
        pnode = self.inodes[pnum]
        pblock = self.data[inode.getAddr()]
        if pblock.getFreeEntries() == 0:
            return -1
        self.createFile(parent, newfile, 's')
